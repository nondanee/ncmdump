...
try:  # install and call
    from ncmdump import dump
except Exception:  # relative call
    pass
...
def main():
    ...
    def name_format(path, meta):
        information = {
            'artist': ','.join([artist[0] for artist in meta.get('artist')])
        ...
        name = os.path.splitext(os.path.split(path)[1])[
            0] if not name else name
