...
class ncm_dump(Thread):
    def __init__(self, file_path, file_name, save_path, q):
        super().__init__()
    ...
    def run(self):
        self.queue.put("正在转化 --- " + self.file_name)
        self.dump(self.file_path, self.save_path)
        # self.queue.join()  # 阻塞，直到任务完成

    def dump(self, file_path, save_path):
